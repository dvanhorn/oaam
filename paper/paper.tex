...
\begin{align*}
force(\clos{x,e,\menv},\msto) &= \{\clos{x,e,\menv}\}\\
force(\saddr\maddralt,\msto) &= \msto(\maddralt)\\
\ev{\svar{x},\menv,\mkont,\msto} &\machstep\; 
\co{\mkont,\saddr{\menv(x)},\msto} \\
\co{\kar[^\mcntr_\mlab]{\mexp,\menv,\maddr},v,\msto}
&\machstep\;
\ev[^\mcntr]{\mexp,\menv,\msto,\kfn[^\mcntr_\mlab]{v,\maddr}} \\
\mbox{where } &v' \in force(v,\msto)
bind^\mcntr_\mlab(\msto,x,\menv,v) =
\end{align*}
Lazy:

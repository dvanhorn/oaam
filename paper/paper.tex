...
application of a widening operator\footnote{Technically, we would have to copy the value of the global store to all states being stepped to fit the formal definition of a widening, but the consequential accelerated convergence fits the M.O. of a widening operator.} during the fixed-point iteration.
%
...
\item{The common implementation strategy for timestamps destructively
    ...
    stores in the seen set ($S$), so we do not experience precision
    loss.}
\end{enumerate}
...
\ap[^\mcntr_\mlab]{\mvalx{u},\mval,\mkont},\epsilon \\

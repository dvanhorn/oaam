...
precision, and is deterministic even if set iteration is not.
...
All but the first technique are precision preserving. We will discuss the design rationale
for the move.
...
\ans{\msto,\mval}
\\
\co{\kar[^\mcntr_\mlab]{\mcomp,\menv,\maddr},\mval,\msto} & \cmachstep
\mcomp^\mcntr(\menv,\msto,\kfn[^\mcntr_\mlab]{\mval,\maddr})
\\
\co{\kfn[^\mcntr_\mlab]{{\mvalx{u}},\maddr},\mval,\msto} & \cmachstep
...
There is two subtle losses of precision in Shivers' original
time-stamp technique that we can fix. Firstly, for a given abstract
state, all writes to the global store need to be delayed until the
analysis considers all branches from that abstract state. This avoids
...
stores this change is straightforward. Secondly, the common
implementation strategy for timestamps destructively updates each
state's timestamp. This loses \emph{temporal} information about the
contexts a state is visited in, and in what order. Thus that semantics
is not equivalent to the previous section's semantics.

A fix is to keep a stack of the timestamps seen at each state, and
keep a list of stores corresponding with each timestamp. A more
space-efficient representation that also allows an imperative store
would be timestamping each update to the store in the same style as
states. This changes the notion of store equality, but the new
definition is straightforward. For analysis consumers that disregard
temporal information, the space cost in unwarranted, but the
realization that there is precision to be gained opens possibilities.
...
algorithms that can be proved correct.

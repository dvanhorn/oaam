...
\allockont^\mcntr_\mlab(\msto,\mkont) &= \maddr \mbox{ where } \maddr \notin \msto
\end{align*}
where $\sqcup$ on stores is a point-wise lifting of $\sqcup$, \ie,
$\msto \sqcup \msto' = \lambda \maddr. \msto(\maddr) \sqcup
\msto'(\maddr)$ and $\mstate.\msto$ is notation for projecting out the
store component of a state. These functions appear to ignore $\mlab$
and $\mcntr$, but they can be used to determinize the choice of fresh

...
Figure~\ref{fig:church} gives a small example of computing with Church
numerals.  The example, while small, is informative: (1) it can be
written in almost any modern programming language\footnote{It is
...
%
and (2) it stresses an analyzer's ability to deal with complicated
environment and control structure arising from the use of higher-order
functions to model arithmetic.

Our baseline analyzer takes NN:NN minutes to analyze this program
(without store widening it times out after 30 minutes).  This is on
par with the performance of Sergey,~et al.'s analyzer, which takes
NN:NN minutes.
...
contributes an obscene fraction of the runtime.

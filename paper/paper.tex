...
the address. This nondeterminism is necessary in order to continue with
evaluation.
...
%
Given two states, the analysis can now compare their stores just by comparing their timestamps -- a
...
\item{In our semantics, the store does not change until the entire
    ...
    which would otherwise happen in Shivers' semantics, e.g., when one branch writes to
...
\item{The common implementation strategy for timestamps destructively
    ...
    order. Our semantics has a drop-in replacement of timestamps for
    stores in the seen set ($\mseentime$), so we do not experience precision
    loss.}
\end{enumerate}
...
\begin{lemma}
  $\widehat{\machstep}$ maintains the invariant that all stores in $\mseen$ are
  totally ordered and $\msto$ is an upper bound of the stores in $\mseen$.
\end{lemma}
...
\end{theorem}
The proof follows from the order isomorphism that, in one direction,
sorts all the stores in $\mseen$ to form $\Sigma$, and translates stores in
$\mseen$ to their distance from the end of $\Sigma$ (their timestamp). In the other
direction, timestamps in $\mseentime$ are replaced by the stores they point to in $\Sigma$.
...
to determine which branch to to take.
%
...
contained in the right-hand-side store, and if values occur in the states,
the left-hand-side value must be in the forced corresponding right-hand-side value.
...
from the previous semantics is that $\evalone$ state construction is

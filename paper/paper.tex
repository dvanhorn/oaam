...
either ANF or CPS. We can also play a representation trick and remove
$\menv$ since we know it is the identity function in 0-CFA. We will
refer to this change in the semantics as value indirection.
...
integer factors. The first three remain in the purely functional
realm. The final three use mutation in small, localized places, so
they do not increase conceptual complexity drastically.
...
quotient the state space by the values at the delayed address. Its changes to the semantics are highlighted in the following rules:
...
variable reference a step later and another binding is not made in
...
with value indirection consumes the value component of states in
...
\ref{fig:compile} shows close similarity to the rules for interpreting
    {\tt ev} states. The next step is to change creation of {\tt ev} states to simply calling these functions. The modified reduction relation is shown in figure \ref{fig:caam}.
...
value of which do not particularly matter for control-flow
analysis. If we interpret a list literal of 30 elements (that again
have 10 elements each) as 300 applications of {\tt cons}, and then
interpret a function that traverses the tree, we will end up
interpreting that function at least 300 times, when once or twice
would do.
...
even in our fastest parameterization, this abstraction causes the
...
\begin{tiny}
\begin{alltt}
Program  |  LOC | B. Time | O. Time | B. Space | O. Space | B. Rate | O. Rate
----------------------------------------------------------------------------
nucleic  | 3508 |
matrix   |  747 |
nbody    | 1434 |
...
The analysis most similar to that presented in section~\ref{sec:aam}
is the infinitary control-flow analysis of Nielson and
Nielson~\cite{dvanhorn:nielson-nielson-popl97} and the unified
treatment of flow analysis by Jagannathan and

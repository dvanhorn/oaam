...
form~\cite{dvanhorn:Danvy:DSc}.\marginpar{There's probably a better
  (more specific) cite for eval/apply/continue.}  It can be
...
when we create the continuation.  This address and the tail address are
...
analyzed. This precision-preserving transformation eliminates
interpretative overhead by first compiling the program into ``byte
code'' instructions.
...
several, commonly large, stores together. Each step is unlikely to
always modify all addresses of the store, so this is needlessly
memory- and time-intensive. We can instead accumulate store changes
and apply them after all steps have completed. This uses far fewer
join operations, leading to much less allocation and computational
overhead.

We represent changes as $Store\msdiff = List[Addr \times
  \Set(Storeable)]$. Each $\msto\sqcup[\maddr \mapsto \mval{s}]$ now
becomes $\cons{\ttuple{\maddr}{\mval{s}}}{\msdiff}$, where $\msdiff$
begins as empty for each step. Applying the changes is
straightforward:
...
\setminus S$ in ${\mathcal G}$), we still have to compare entire
stores - an expensive computation.  We modify a technique due to
Shivers~\cite{ianjohnson:Shivers:1991:CFA}, timestamp approximation,
that removes the need for joining whole stores by manipulating a
global store imperatively. The original technique loses precision
because states are stepped in sequence, modifying the store in place -
more values or continuations can be added before another state gets to
step. This interference means more possible flows (i.e. less
precision) and a nondeterministic analysis - the order the set of states is
traversed is not necessarily deterministic. However, by delaying the
store changes until after all steps have completed, we can maintain
our precision and determinism. In a variety of experiments, we
observed that the time and space overhead for storing store-deltas was outweighed by
the precision keeping flows in check. That is to say, the analysis was
faster {\it and} more precise.

We pair this technique with a mutable workset to avoid the
intermediate sets introduced by the $State \times Store \to \Set(State) \times Store\msdiff$ signature
of $\machstep$. This necessitates a change to the fixpoint algorithm to use a global
workset, {\tt todo}, that $\machstep$ imperatively updates with the
states that it would have otherwise accumulated in a set that would later be added to the workset.
...
equivalent to an interreter for the language, just as is the case

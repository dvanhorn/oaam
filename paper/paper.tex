\documentclass[preprint,onecolumn,9pt]{onecol} %{sigplanconf}
\usepackage{alltt}
...
garbage collection.  It has since been used to verify actor-based
parallelism~\cite{local:DOsualdo:12A} and behavioral
contracts~\cite{FIXME:tobin-hochstadt}; it has been used to model
Dalvik, Erlang~\cite{local:DOsualdo:12B},
JavaScript~\cite{local:DBLP:journals/corr/abs-1109-4467}, Racket, and
...
\begin{align*}
force(\clos{\mvar,\mexp,\menv},\msto) &= \{\clos{\mvar,\mexp,\menv}\}\\
force(\saddr\maddralt,\msto) &= \msto(\maddralt)\\
\ev{\svar{\mvar},\menv,\mkont,\msto} &\machstep\; 
\co{\mkont,\saddr{\menv(\mvar)},\msto} \\
\co{\kar[^\mcntr_\mlab]{\mexp,\menv,\maddr},v,\msto}
&\machstep\;
\ev[^\mcntr]{\mexp,\menv,\msto,\kfn[^\mcntr_\mlab]{\mval,\maddr}} \\
\mbox{where } &\mval' \in force(\mval,\msto) \\
bind^\mcntr_\mlab(\menv,\msto,\mvar,\mval) &= \menv[\mvar \mapsto
  \maddr], \msto\sqcup[\maddr \mapsto force(\mval, \msto)], \mcntr' \\
\mbox{where } \mcntr' &= \lfloor\mlab\mcntr\rfloor_k \\
              \maddr &= \mvar\mcntr'
\end{align*}
...
in fact equal. We do this by showing each individual reduction takes
equal stores to equal stores, and that the union of all these
reductions is the full reduction.
...
%% By definition of force, v in force(addr(r(x)), s)
...
%% Premises:
%% For all v' in force(v, s),
%%   <v' ar^{l,lf,la}(e^le r a) d> in conf.cs
%% Thus:
%% <v ar^{l,lf,la}(e^le r ka) d> -->lazy   <e^le r s\sqcup[lfd |-> force(v, s)] fn^{l,la}(lfd ka) d>
%% for all v' in force(v, s),

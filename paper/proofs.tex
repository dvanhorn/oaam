...
semantics is a WEB refinement of the non-compiled semantics. We equate
states that ``commit'' to non-$\ev{}$ states.
...
state. As a corollary, $r$ is idempotent.

Next we relate states across the different machines with an
equivalence relation $B$ on the two state spaces $S = \State \cup \CompState$, such that $\forall \mstate \in \State. s B r(s)$:
\begin{align*}
(s,s') \in B \iff r(s) = r(s')
\end{align*}
...
...}
\end{byCases}
% (a) case is for non-ev to non-ev

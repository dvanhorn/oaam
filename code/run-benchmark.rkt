  ...
  (parameterize ([current-logger (make-logger 'stuck-states)])
    ...
    (with-limit-handler ;; Prints state rate even if timeout/oom
     (with-limits (* 30 #;run-for-30-minutes
                  ...
                  ;; TODO: delineate implementation names into different modules
                  ;; and dynamically require instead.
                  (let ([analyze (dynamic-require "kcfa-instantiations.rkt" (aval))])
                    (call-with-values (λ ()
                                         (begin0 (time (analyze e))
                                           (void)
                                           (dump-memory-stats)
                                           (flush-output)

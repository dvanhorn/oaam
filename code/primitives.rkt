...
(define-for-syntax prim-defines
  #'((define both '(#t #f))
     ...
     (define/basic (stringv? v) `(,(or (string? v) (eq? 'string v))))
     ...
     (define/write (make-vectorv σ l δ size [default 0])
       (match (widen size)
         ...
         [_ (define V-addrs
            ...
            (do (σ) ([σ* #:join* σ V-addrs (make-list size default)])

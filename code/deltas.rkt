...
(define-syntax-rule (mk-∆-fix^ name ans^?)
  (define-syntax-rule (name step fst)
    (let-values ([(∆ cs) fst])
      ...
      (define-values (Σ ss) (fix-t2 step^ (update ∆ empty-σ) cs))
      (state-rate)
      (define final-cs
        (for/fold ([final-cs ∅]) ([s ss])
          (match s
            [(cons fsσ c)
             (∪ final-cs (if (ans^? c) (set c) ∅))]
...
(define-syntax-rule (mk-∆-fix2^ name ans^?)
  (define-syntax-rule (name step fst)
    (let-values ([(∆ cs) fst])
      ...
      (set-box! (start-time) (current-milliseconds))
      (define ss (fix step^ (set (cons (update ∆ empty-σ) cs))))
      (state-rate)
  ...
  #,(with-syntax ([ans^? (format-id #'ans^ "~a?" #'ans^)])
      #'(define-syntax-rule (name step fst)
          (let ()
            ...
            (define-values (last-σ final-cs)
              (let loop ([accum (hash)] [front cs] [σ (update ∆ empty-σ)] [σ-count 0])
                (cond [(∅? front)
                       (state-rate)
